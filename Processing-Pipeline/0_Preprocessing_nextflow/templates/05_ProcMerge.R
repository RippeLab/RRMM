...
seurat_object <- FindVariableFeatures(seurat_object,assay="SCT", nfeatures = 3000)
# Run PCA and UMAP
...
DimPlot(seurat_object, label = TRUE, group.by = "patient") + NoLegend()
dev.off()

pdf(file.path(merged_plots_path, "Sorting_umap.pdf"), height = 8, width = 8)
DimPlot(seurat_object, label = TRUE, group.by = "sorting") + NoLegend()
dev.off()

pdf(file.path(merged_plots_path, "Sorting_hyperdiploidy.pdf"), height = 8, width = 8)
DimPlot(seurat_object, label = TRUE, group.by = "hyperdiploidy") + NoLegend()
dev.off()
...
DimPlot(seurat_object, group.by = "celltype_1_predicted", cells.highlight = names(colnames(seurat_object)[which(is.na(seurat_object[["celltype_1_predicted"]][[1]]))]), label = T) +
    NoLegend() +
...
qsave(seurat_object, "all_patients_merged_umap.qs", nthread = 4)
